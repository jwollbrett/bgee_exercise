...
public class DetectHomologyEntities {
	...
	public static Map<String, HomoStructWithAncestorsBean> detectHomologieswithAncestorsThroughProperty(OWLOntology ontology, Set<OWLClass> owlClassesWithAnnotations,Set<OWLPropertyExpression> properties){
		OWLGraphWrapperEdgesExtended graphWrapper = new OWLGraphWrapperEdgesExtended(ontology);
		 ...
		 while(it.hasNext()){
			 ...
			 //Set used to count both number of distinct species and number of distinct ancestors
			 ...
			 for(OWLGraphEdge edge:subGraphEdgeSet){
				 if(edge.getGCIFiller()!=null&&properties.contains(edge.getSingleQuantifiedProperty().getProperty())){
	...
	private static OWLClass fromTargetOWLObjectToOWLClass(OWLObject targetObject){
		...
		if(targetClassSet.size()==1){
			...
			Logger.getLogger(DetectHomologyEntities.class).error("Can't find target Class because the target OWLObject contains more than one OWLClass");
		}
		return targetClass;
	}
}
